...
class KeyStore {
    ...
    putValue(value, newKey = uuidv4()) {
        if (this.keyExists(newKey)) {
            throw new KeyStoreError("Tried to overwrite value, key already exists in store. (use .replaceValue).");
        }
    ...
    reverseLookUpExactlyOneOrThrow(value) {
        ...
        if (candidates.length !== 1) {
            throw new KeyStoreError(`Key: '${value}' has ${candidates.length} matches. Can not determine 1:1 mapping.`);
        }
        return candidates[0];
    }
    /**
