...
class TreeObfuscator<T> implements ITree<T> {
  ...
  getChildContentAt(nodeKey: string): TGenericNodeContent<T> {
    const nodeId = this.getNodeId(nodeKey);
    if (nodeId) {
  ...
  getChildrenContentOf(nodeKey: string): TGenericNodeContent<T>[] {
    const nodeId = this.getNodeId(nodeKey);
    if (nodeId) {
      return this._internalTree.getChildrenContentOf(nodeId);
    }
    return [];
  }

  getChildrenNodeIdsOf(nodeKey: string): string[] {
    const nodeId = this.getNodeIdOrThrow(nodeKey);
  ...
  getDescendantContentOf(nodeKey: string): TGenericNodeContent<T>[] {
    const nodeId = this.getNodeId(nodeKey);
    if (nodeId) {
      return this._internalTree.getDescendantContentOf(nodeId);
    }
    return [];
  }
