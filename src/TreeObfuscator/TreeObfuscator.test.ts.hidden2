...
import { TPredicateNodeTypes } from "../../dev-debug/types";
...
// checkIdMapsToCorrectObject
class TestAbstractDirectedGraph extends AbstractExpressionTree<PredicateNodeTypes> {
...
describe("TreeObfuscator", () => {
  describe("Creation", () => {
    it("Should append/get child (blue skies).", () => {
      ...
      expect(obfuscatedGraph.getChildContentAt(childId_0)).toBe(predicate0);
    });
  });
  describe(".move(...) ", () => {
    it("Should move node. Preserve original target nodeId content relationship. (Leaf)", () => {
      ...
      const x = dTree.getChildrenContentOf(targetNodeId);
      // pre conditions
      expect(dTree.getChildrenContentOf(targetNodeId).sort()).toStrictEqual(
      ...
      expect(dTree.getChildrenContentOf(targetNodeId).sort(SortPredicateTest)).toStrictEqual(
        [
          // OO["child_1"],
          OO["child_1_0"],
          OO["child_1_1"],
    ...
    it("Should move node. Preserve original target nodeId content relationship. (Branch)", () => {
      ...
      // pre conditions
      expect(dTree.getDescendantContentOf(targetNodeId).sort(SortPredicateTest)).toStrictEqual(
        [OO["child_1_0"], OO["child_1_1"], OO["child_1_2"]].sort(
      ...
      // post conditions
      const x = dTree.getDescendantContentOf(targetNodeId).sort(SortPredicateTest);
      expect(dTree.getDescendantContentOf(targetNodeId).sort(SortPredicateTest)).toStrictEqual(
        (
          [
            // { operator: "$or" },
  ...
  describe(".moveChildren(...)", () => {
    it("Should attach children of source to target and remove source Node", () => {
      ...
      const preTargetDescendantContentOf = dTree.getDescendantContentOf(targetNodeId);
      expect(preTargetDescendantContentOf.sort(SortPredicateTest)).toStrictEqual(
        [OO["child_1_0"], OO["child_1_1"], OO["child_1_2"]].sort(SortPredicateTest)
      );

      const preSourceDescendantContent = dTree.getDescendantContentOf(sourceNodeId);
      ...
      expect(dTree.getDescendantContentOf(sourceNodeId).sort(SortPredicateTest)).toStrictEqual(
        [].sort(SortPredicateTest)
      );
  ...
  describe("SubTree POJO to/from", () => {
    ...
    it("Should be able to create subTree export/import.", () => {
      ...
      const newTree = AbstractDirectedGraph.fromPojo<
        ...
        // @ts-ignore - tree pojo typings
      >(subtreePojo);
      expect(newTree.getChildContentAt(newTree.rootNodeId)).toStrictEqual(OO["child_0"]);
  ...
  describe("AbstractDirectedGraph.fromPOJO", () => {
    ...
    it("(Predicate not WidgetType)Should create subtrees if Pojo contains nodeType::subtree.", () => {
  ...
  describe("Miscellaneous, pass-through", () => {
    it.only("Should be an alias for the underlying class.", () => {
      const originalGraph = new TestAbstractDirectedGraph(undefined);
      const obfuscatedGraph = new TreeObfuscator(originalGraph);
      const { dTreeIds, dTree, subgraph } =
        make3Children9GrandchildrenITreeWithSubgraph_2_3(obfuscatedGraph);
